...
- ✅ **Safe Iteration Changes**: `dryRun` previews, disk-backed assignment snapshots, and per-iteration restore reporting
...
> **Assignments are preserved automatically.** GitHub has no per-iteration mutation — the whole configuration must be replaced, and that regenerates every iteration ID, detaching all item assignments. Both `add_iteration` and `update_iteration` snapshot every item→iteration value first and re-apply it by iteration title afterwards. The result includes `assignmentsRestored: { restored, failed, byIteration }` so a partial restore is visible rather than silent, and `byIteration` answers *restored to where* without a separate read of the board.

> **The snapshot is written to disk before the mutation.** Path is returned as `snapshotPath`, under `~/.config/github-projects-mcp/iteration-snapshots/`. The restore is one mutation per assigned item, so on a large board the window between "assignments detached" and "assignments restored" is minutes long; if the process dies inside it, the values are not recoverable from the API. Replay the file with [`restore_iteration_snapshot`](#restore_iteration_snapshot). If the snapshot cannot be written, the call fails **before** mutating anything.

> **Iteration titles must be unique.** Restore is keyed by title — the only identifier that survives the mutation — so a duplicate would silently reassign items to the wrong iteration. A call that would leave two iterations sharing a title is refused, naming the offender.
...
- `dryRun` (boolean, optional): Report what the call would do and mutate nothing
...
Update an existing iteration's title, start date, or duration. Item assignments are preserved, the snapshot is written to disk first, and titles must stay unique — see the notes under [`add_iteration`](#add_iteration). Renaming is handled too: items on the renamed iteration are remapped to the new title rather than dropped, and `dryRun` reports the rename it detected so you can confirm the remap engaged.
...
### `dryRun` on iteration changes

Both `add_iteration` and `update_iteration` accept `dryRun: true`. Nothing is mutated and no snapshot file is written; the two read queries run and the report comes back:
...
```jsonc
{
  ...
  "iterationsBefore": [
    { "id": "bab3ba50", "title": "Phase 7: A", "startDate": "2026-08-17", "duration": 14 }
  ],
  // As they would be. No ids — every one is regenerated by the mutation.
  "iterationsAfter": [
    { "title": "Phase 7: B", "startDate": "2026-08-17", "duration": 14 }
  ],
  // Non-null when a title change is detected, so you can confirm the remap engaged.
  "rename": { "from": "Phase 7: A", "to": "Phase 7: B", "assignmentsRemapped": 1 },
  // Keyed by the titles the restore will use, post-remap.
  "assignments": { "total": 24, "byIteration": { "Phase 7: B": 1, "Phase 6": 23 } }
}
```

Diffing `assignments.byIteration` from the dry run against `assignmentsRestored.byIteration` from the real call is the strongest verification available without a transaction.

---

### `restore_iteration_snapshot`

Re-apply a snapshot written by `add_iteration` or `update_iteration`. Use it when a call reported entries under `assignmentsRestored.failed`, or when a call was interrupted before its restore finished. Idempotent — re-applying an assignment an item already has is a no-op.

**Parameters:**
- `snapshotPath` (string, required): Path from the `snapshotPath` field of an `add_iteration` or `update_iteration` response

**Example:**
```typescript
{
  "snapshotPath": "/Users/you/.config/github-projects-mcp/iteration-snapshots/PVT_kwHOAwJiCM4BNC20-2026-08-28T12-34-56-789Z.json"
}
```

Assignments are re-applied against the field's *current* iteration ids, read fresh — the ids in the snapshot are only there for a human reading the file.
